...
def write_node(state:WriteState) -> WriteState:
    """카피를 최초 또는 피드백에 의거 재작성하는 노드"""
    count = state.count

    if not state.draft: # draft 가 없으면 최초 작성으로 인식
        prompt=f"""
            당신은 건강하고 멋진 광고카피를 만들어내는 전문 카피라이터 입니다.
            [주제]에 대해서 한 문장으로 된 멋진 광고카피를 작성하세요.
            불필요한 설명 없이 오직 카피문구만 출력하세요.
            [주제] : {state.topic}
        """
    else:
        prompt=f"""
            [피드백]을 반영하여 [광고카피] 내용을 수정해주세요.
            불필요한 설명 없이 오직 카피문구만 출력하세요.
            [피드백] : {state.feedback}
            [광고카피] : {state.draft}
        """

    count += 1  # 글을 한 번 쓸때마다 count 가 1 증가
...
def route_by_review(state:WriteState) -> str:
    """PASS / RETRY 에 따라서 다른 노드로 갈 수 있는 문자열을 반환"""
...
wf.add_conditional_edges(
    ...
    route_by_review,
    {
        'go_end':END, # END는 ctrl+space 하여 자동완성 선택(langgraph)
        'go_retry':'writer'
    })
