...
class AgentState(TypedDict):
    # lang Graph 에서는 상태를 덮어쓰는 것을 원칙으로 하고 있다.
...
def tool_node(state:AgentState) -> Dict:
    ...
    # 메세지들 중에서 직전의(마지막) 메세지인 AIMessage 를 가져온다.
    ...
    for call in last_msg.tool_calls:
        ...
        nsg_list.append(ToolMessage(content=str(result),tool_call_id=call_id))

    return {'messages':nsg_list}

def should_continue(state:AgentState) -> str:
    """LLM 최근 메세지에서 tool_calls 가 있으면 call_tool 로, 아니면 go_end 로 반환한다."""
    last_msg = state['messages'][-1]
...
for node in app.stream(
        {'messages':[HumanMessage(content="256 곱하기 4가 무엇인지 계산해 주세요")]},
        stream_mode="updates"):
    for key,val in node.items():
        print(f'[{key}]     {val}')

"""
HumanMessage    : 사용자가 보내는 메세지(content)
AIMessage       : LLM 모델이 생성한 메세지(content,tool_calls)
ToolMessage     : Tool 이 수행후 반환하는 메세지(content,tool_call_id)
"""
