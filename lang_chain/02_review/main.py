...
def get_structured(review:str):
    # 파이프라인 조립. structured_model 내 model 이 포함되어 있음
    chain = prompt | structured_model
    # 출력
    result = chain.invoke({'review':review})
    print(result)
    # 일반적으로는 model_dump() / 보내는 내용이 복잡하면 model_dump_json()
    # return result.model_dump_json() # jsom 형태의 문자열(받는쪽에서 JSON.parse() 써줘야 한다.)
    return result.model_dump() # dict 형태로 변환 -> fastapi 에선 dict 로 변환하면 json 으로 자동 변환