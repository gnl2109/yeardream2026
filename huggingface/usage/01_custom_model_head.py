"""
    ...
    Head - 받은 임배딩 내용을 우리가 원하는 최종 결과로 바꿔주는 부분
"""
import torch
...
class CustomClassifier(PreTrainedModel):
    ...
    def __init__(self, config): #생성자(클래스가 객체화될 때 가장 먼저 실행
    ...
    # model() 하면 forward() 가 실행된다.(즉, 실행함수)
...
[-0.0040, -0.1406]
"""
# dim=1|-1 : 가로방향으로 연산, dim=0|1 : 세로방향으로 연산
...
# 모델 저장. 불러오는 방법은 모델 불러올때 저장된 경로를 입력해주면 됨
