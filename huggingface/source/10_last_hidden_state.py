...
# 4. 모델 추론
with torch.no_grad():
    # output_hidden_states=True 로 하면 모든 레이어의 hidden state 를 받는다.
...
for idx, token in enumerate(tokens):
    ...
    # 768개의 배열에는 무엇이 들어있는가?
    # 단어를 구분할 수 있는 기준 조건(Feature)
    # 단일 단어 뿐만 아니라 인근의 단어들과의 관계도 포함이 된다.

# last_hidden_state 는 문장안의 모든 각 단어의 의미를 담은 벡터를 확인할 때 사용