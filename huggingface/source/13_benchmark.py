...
def benchmark(attn_type,name):
    ...
    # CPU 가 GPU 작업 종료까지 기다리도록 동기화 시켜준다.
    torch.cuda.synchronize()
    end_time = time.time() - start_time
    print(f'=== [{name}] 이 걸린시간 : {end_time} ===')

# benchmark('sdpa','Flash Attention 방식(SDPA)') # 0.3269317150115967
benchmark('eager','Standard Attention 방식(EAGER)') # 0.3226916790008545